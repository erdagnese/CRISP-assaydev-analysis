...
```{r vis of specificity}
ggplot(Rs_spec, aes(x=Cycle, y=FAM, color = sample)) + geom_point()
```
```{r visualisation of sensitivity 1}
ggplot(Rs_sens1, aes(x=Cycle, y=FAM, color = sample)) + geom_point()

```
```{r vis for sensitivity 2 run}
ggplot(Rs_sens2, aes(x=Cycle, y=FAM, color = sample)) + geom_point()

```
In case there are positive NTC values, it is a good idea to calculate these, but is not necessary for the analysis if all NTCs remain negative.
...
```{r derivative - cumulative sum - specificity}
Rs_spec %>%
  group_by(Well) %>%
...
```{r specificity with 0 as threshold}
Rs_spec %>%
arrange(Species) %>%
...
```{r sensitivity run 2 cumulative sum}
Rs_sens2 %>%
  group_by(Well) %>%
  mutate(FAMcs = cumsum(FAM))  %>%
...
```{r combine all sensitivity data}
...
```
Need to remove the isolates 33209 isolates as they were seeded in water and frozen which appears to be an issue when extraction, multiple extractions failed
```{r plotting of all sensitivity from cultures}
Rs_sens_all %>%
...
ggsave(filename = "Rs_sens_culture_sensitivity.png", device = 'png', dpi = 700, width = 8, height = 6)

```
Remake as facetted graphs
```{r plot facetted by isolate for ease of visualization}
Rs_sens_all_cult %>%
...
```{r pull out positive for time to detection}
Rs_sens_all_cult %>%
  group_by(Well,sample) %>%
  #filter(FAM > 0) %>%
  mutate(CtPos = FAM > 0) %>%
...
```{r qPCR vs CRISPR df for single isolate}
Rs_sens_all_cult %>% 
...
```{r join qPCR data with CRISPR data}
...
```{r time to detection with copies/ul plot}
Rs_16_compare %>%
  group_by(Well) %>%
  #filter(FAM > 0) %>%
  mutate(CtPos = FAM > 0) %>%
...
Alternate plot without concentration in black and white
```{r copies/ul vs time to detect BW}
...
test whether time to detection is related to concentration (not working right at moment)
```{r}
Rs_16_compare %>%
  group_by(Well) %>%
  #filter(FAM > 0) %>%
...
```{r visualize time to detection trend for all sensitivity}
Rs_sens_BgCor %>%
  #filter(CorCt == "Y") %>%
  mutate(isolate = case_when(
...
```{r eDNA detections fro 0 threshold}
Rs_eDNA_all %>%
...
```{r agreements between CRISPR and qPCR}
Rs_eDNA_both_Ct %>%
...
combine the detections and export figure
```{r plot of eDNA results}
